...
t = table('bin/top25.data')

# create the postscript file we'll use as our canvas
canvas = postscript('imgs/top_mods.eps')
...
d = drawable(canvas, xrange=[-0.5,t.getmax('rownumber')+0.5], yrange=[0,6])
...
axis(d, xtitle='Module', xmanual=t.query(select='year,rownumber'),
     ytitle='Number of Imports', yauto=[0,6,1], xlabelrotate=45, xlabelfontsize=6.5)
