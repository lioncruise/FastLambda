...
t = table('bin/counts.data')

# create the postscript file we'll use as our canvas
canvas = postscript('imgs/counts.eps')
...
d = drawable(canvas, xrange=[-0.5,t.getmax('rownumber')+0.5], yrange=[0,400])
...
axis(d, xtitle='Year', xmanual=t.query(select='year,rownumber'),
     ytitle='Repositories (Thousands)', yauto=[0,400,100], xlabelrotate=45, xlabelfontsize=6.5, ylabelfontsize=6.5)
