...
t = table('bin/watchers.data')

# create the postscript file we'll use as our canvas
canvas = postscript('imgs/watchers.eps')
...
d = drawable(canvas, xrange=[-0.5,t.getmax('rownumber')+0.5], yrange=[0,30])
...
axis(d, xtitle='Year', xmanual=t.query(select='year,rownumber'),
     ytitle='Avg. Watchers', yauto=[0,30,10], xlabelrotate=45, xlabelfontsize=6.5)
